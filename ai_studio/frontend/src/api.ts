...
/** The QA gate report. Everything except the pending/message pair is optional
 *  because a draft project answers 200 {pending: true} with no findings at all —
 *  the panel reads these unguarded, so the type has to admit that. */
export interface QAResult {
  ...
  /** what is missing, or what the gate could not do — one actionable sentence */
  message?: string;
  /** findings the panel has to render (failures + warnings) */
  checked?: number;
