...
# ─────────────────────── clips · image search · sfx (one server) ───────────────────
# These three surfaces used to live on two other web apps (:8001 Auto-Clip
# dashboard, :8002 AI Creator page). Those pages are deleted; the engines behind
# them are kept and are reached through the studio API, so there is exactly one UI
# and no feature that only existed somewhere else.

_CLIPS_ENGINES: dict = {}


def _clip_engine():
    """One ClipEngine per data root (tests use temp roots; prod uses data/studio)."""
    st = get_state()
    eng = _CLIPS_ENGINES.get(st.data_root)
    if eng is None:
        from . import clips as clips_mod
        eng = _CLIPS_ENGINES.setdefault(st.data_root, clips_mod.ClipEngine(st.data_root))
    return eng


def _clip_llm_options() -> dict:
    """The analyzer's LLM step uses the STUDIO's own Ollama settings — the legacy
    dashboard kept a second config.json, and two answers for one machine is how
    'why did the model change?' bugs start."""
    cfg = get_state().config()
    roles = cfg.get("roles") or {}
    script = roles.get("script") or {}
    host = (cfg.get("ollama") or {}).get("host") or ""
    if not script.get("enabled", True) or not host:
        return {"llm_provider": "Off"}
    return {"llm_provider": "ollama", "ollama_model": script.get("model") or "",
            "ollama_host": host}


@router.get("/clips")
async def api_clips():
    """Everything the clip panel needs at load: capability truth, files, jobs."""
    from . import clips as clips_mod
    eng = _clip_engine()
    return {"engine": clips_mod.availability(os.getcwd()),
            "videos": eng.list_videos(), "outputs": eng.list_outputs(),
            "jobs": eng.jobs.list()[:20]}
...
async def api_clips_upload(file: UploadFile = File(...)):
    ...
    raw = await file.read()
    try:
        meta = eng.ingest(file.filename or "", raw)
    except ValueError as e:
        raise HTTPException(400, str(e))
    return {**meta, "note": f"stored under {os.path.relpath(eng.upload_dir, get_state().data_root)} "
                            f"in the studio data folder"}


@router.get("/clips/source/{name}")
async def api_clips_source(name: str):
    eng = _clip_engine()
    try:
        path = eng.source_path(name)
    except (FileNotFoundError, ValueError) as e:
        raise HTTPException(404, str(e))
    return _stream_with_range(path, "video/mp4")


@router.post("/clips/analyze")
async def api_clips_analyze(payload: dict = Body(default={})):
    from . import clips as clips_mod
    if not clips_mod.availability(os.getcwd())["available"]:
        raise HTTPException(503, "the clip engine cannot run here: "
                                f"{', '.join(clips_mod.availability(os.getcwd())['missing'])} "
                                f"is not installed — {clips_mod.availability(os.getcwd())['install']}")
    eng = _clip_engine()
    video = str(payload.get("video") or "")
    try:
        jid = eng.analyze(video, top_n=int(payload.get("top_n") or 5),
                          min_s=int(payload.get("min_seconds") or 15),
                          max_s=int(payload.get("max_seconds") or 30),
                          use_whisper=bool(payload.get("use_whisper")),
                          whisper_model=str(payload.get("whisper_model") or "small"),
                          llm=_clip_llm_options())
...
@router.get("/clips/highlights/{name}")
async def api_clips_highlights(name: str):
    eng = _clip_engine()
    try:
        eng.source_path(name)
    except (FileNotFoundError, ValueError) as e:
        raise HTTPException(404, str(e))
    clips = eng.highlights(name)
    return {"video": name, "clips": clips,
            "note": "" if clips else "not analyzed yet — press analyze and the candidate "
                                     "clips appear here"}


@router.post("/clips/export")
async def api_clips_export(payload: dict = Body(default={})):
    eng = _clip_engine()
    try:
        jid = eng.export(str(payload.get("video") or ""), int(payload.get("index") or 0),
                        caption_style=str(payload.get("caption_style") or "Reels"),
                        track_faces=bool(payload.get("track_faces", True)),
                        voiceover_text=str(payload.get("voiceover_text") or ""),
                        use_kokoro=bool(payload.get("use_kokoro")),
                        kokoro_voice=str(payload.get("kokoro_voice") or ""))
    except FileNotFoundError as e:
        raise HTTPException(404, str(e))
    except ValueError as e:
        raise HTTPException(400, str(e))
...
@router.get("/clips/jobs/{job_id}")
async def api_clips_job(job_id: str):
    j = _clip_engine().jobs.get(job_id)
    if not j:
        # jobs live in memory only: after a restart the honest answer is "that job
        # is gone", not a spinner stuck at its last percentage
        raise HTTPException(404, "that clip job is not in this server's memory "
                                 "(it finished before a restart) — check the outputs list")
    return j


@router.get("/clips/file/{name}")
async def api_clips_file(name: str):
    eng = _clip_engine()
    try:
        path = eng.out_path(name)
    except (FileNotFoundError, ValueError) as e:
        raise HTTPException(404, str(e))
    mime = "audio/x-wav" if path.endswith(".wav") else ("application/x-subrip"
            if path.endswith(".srt") else "video/mp4")
    return FileResponse(path, media_type=mime, filename=os.path.basename(path))


@router.post("/images/search")
async def api_image_search(payload: dict = Body(default={})):
    """Find (or draw) one supporting still for a scene.

    The legacy page's rule is kept: only CC / public-domain results are accepted,
    and when the web is unreachable the caller is told the image is a locally drawn
    placeholder — never passed off as a found photo.
    """
    st = get_state()
    query = khmer.clip_clusters(khmer.strip_emoji_and_marks(str(payload.get("query") or "")), 120)
    if not query:
        raise HTTPException(400, "an image needs a search phrase")
    cache = ensure_dir(os.path.join(st.data_root, "image-cache"))
    try:
        from ai_creator.image_search import fetch_supporting_image
    except Exception as e:                                     # noqa: BLE001
        raise HTTPException(503, f"the image-search engine is not importable here: {e}")
    res = await asyncio.to_thread(fetch_supporting_image, "web", query, cache)
    fn = os.path.basename(str((res or {}).get("local_path") or ""))
    if not fn or not os.path.exists(os.path.join(cache, fn)):
        return {"found": False, "query": query,
                "note": "nothing was written — the search could not reach an open-licence "
                        "source and the placeholder renderer failed"}
    return {"found": True, "query": query, "file": fn,
            "url": f"/api/images/cache/{fn}",
            "source": (res or {}).get("source") or "web",
            "note": {"web": "Creative-Commons / public-domain result, cached in the studio data dir",
                     "ai": "no CC hit within the timeout — this is a locally drawn placeholder, "
                           "not a photograph"}.get((res or {}).get("source") or "", "cached copy")}


@router.get("/images/cache/{name}")
async def api_image_cached(name: str):
    path = os.path.join(get_state().data_root, "image-cache", os.path.basename(name))
    if not os.path.isfile(path):
        raise HTTPException(404, "that cached image is not here (was the data folder cleaned?)")
    return FileResponse(path, media_type="image/png")


@router.get("/sfx")
async def api_sfx_library():
    """The shared sound-effects folder, so the browser and the render use one list."""
    root = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "sfx_library")
    try:
        from ai_creator.sfx import ensure_library, list_sfx
    except Exception as e:                                     # noqa: BLE001
        raise HTTPException(503, f"the sfx library is not importable here: {e}")
    try:
        ensure_library(root)
    ...
    rows = list_sfx(root)
    for r in rows:
        r["url"] = f"/api/sfx/{r['name']}.wav"
    return {"dir": root, "sfx": rows}
...
async def api_sfx_play(name: str):
    root = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "sfx_library")
    try:
        from ai_creator.sfx import load_sfx
    except Exception as e:                                     # noqa: BLE001
        raise HTTPException(503, f"the sfx library is not importable here: {e}")
    samples, sr = load_sfx(root, os.path.basename(name))
    if samples is None:
        raise HTTPException(404, f"no '{name}' in the sfx folder")
    from .util import wav_bytes
    return Response(content=wav_bytes(samples, sr=int(sr)), media_type="audio/wav",
                    headers={"Content-Disposition": f'attachment; filename="{name}.wav"'})


_GALLERY_PROBE_CACHE = {}


def _gallery_probe(path):
    """Probe a finished cut, remembering the answer until the file changes.

    The page this endpoint replaced cached its footer label the same way, and the reason is
    measured rather than stylistic: opening every export with ffprobe on each drawer render
    made the gallery the slowest screen in the app. The key is (mtime, size), so re-encoding a
    project re-measures it — a cache that lies about a video is worse than no cache at all.
    """
    try:
        key = (os.path.getmtime(path), os.path.getsize(path))
    except OSError:
        return {}
    hit = _GALLERY_PROBE_CACHE.get(path)
    if hit and hit[:2] == key:
        return hit[2]
    from . import media as media_mod
    info = media_mod.probe(path)
    if info.get("probe_used"):
        _GALLERY_PROBE_CACHE[path] = (key[0], key[1], info)
    return info


@router.get("/gallery")
async def api_gallery(probe: bool = Query(False), limit: int = Query(60, ge=1, le=200),
                      project_id: str = Query("")):
    """Every finished cut on this machine.

    This replaces a hand-written page that showed three named renders and had its
    own CSS inside `app.py`: a card here can only exist if a run actually registered
    a `final` asset, and a file that has since been cleaned out of the data folder is
    labelled `missing` instead of quietly vanishing (a dead <video> is far more
    useful than a blank page when someone is hunting for their export).
    """
    st = get_state()
    rows = st.db.list_assets(kind="final", project_id=(project_id or None), limit=limit)
    items, probed = [], 0
    for a in rows:
        pid = a["project_id"]
        proj = st.db.get_project(pid) or {}
        path = a.get("path") or ""
        meta = a.get("meta") or {}
        card = {"asset_id": a["id"], "project_id": pid, "title": proj.get("title") or "(deleted project)",
                "content_type": proj.get("content_type") or "", "run_id": a.get("run_id") or "",
                "created_at": a.get("created_at") or 0, "duration": a.get("duration") or 0,
                "size_bytes": a.get("size_bytes") or 0,
                "width": meta.get("width"), "height": meta.get("height"),
                "captions": meta.get("captions") or meta.get("caption_preset") or "",
                "stream_url": f"/api/assets/{a['id']}/stream",
                "download_url": f"/api/assets/{a['id']}/download",
                "poster_url": "", "missing": not os.path.exists(path), "verified": False}
        post = (st.db.list_assets(project_id=pid, kind="poster", limit=1) or [None])[0]
        if post:
            card["poster_url"] = f"/api/assets/{post['id']}/stream"
        if probe and not card["missing"]:
            info = _gallery_probe(path)
            if info.get("probe_used"):
                card.update({"duration": info.get("duration") or card["duration"],
                             "width": info.get("width"), "height": info.get("height"),
                             "fps": info.get("fps"), "has_audio": info.get("has_audio"),
                             "size_bytes": info.get("size_bytes") or card["size_bytes"],
                             "verified": True})
                probed += 1
        items.append(card)
    items.sort(key=lambda c: -float(c["created_at"] or 0))
    return {"items": items, "total": len(items),
            "from_record": sum(1 for c in items if not c["verified"]),
            "probed": probed,
            "note": ("specs come from the render record; ?probe=1 opens each file with ffmpeg and "
                     "marks it verified (measured files are cached until the file changes)" if not probe else
                     f"{probed} of {len(items)} files opened and measured by ffmpeg")}
...
@router.post("/projects/{project_id}/scenes/{idx}/image-cache")
async def api_scene_image_from_cache(project_id: str, idx: int, payload: dict = Body(...)):
    """Use one already-cached image (from /api/images/search) as a scene's still.

    Only files inside ``<data>/image-cache`` are accepted — the studio never fetches
    a URL a client names, so this cannot be turned into a proxy. The stored result is
    byte-identical to what the upload route would have written, and the scene's
    ``visual_source`` flips to illustration so the video stage skips generation.
    """
    st = get_state()
    if not st.db.get_project(project_id):
        raise HTTPException(404, "project not found")
    ...
    src = os.path.join(st.data_root, "image-cache", name)
    if not name or not os.path.isfile(src):
        raise HTTPException(404, "that cached image is not in this data folder — search again")
    scene_dir = ensure_dir(os.path.join(st.data_root, "projects", project_id, "scenes",
                                        f"{int(idx):02d}"))
    dst = os.path.join(scene_dir, "00_custom.png")
    try:
        await asyncio.to_thread(_normalize_image, src, dst)
    except Exception as e:                                        # noqa: BLE001
        raise HTTPException(400, f"image could not be processed: {str(e)[:160]}")
    meta = dict(scene.get("meta") or {})
    meta["visual_source"] = "illustration"
    meta["image_origin"] = f"image-search:{name}"
    _set_scene_meta(st.db, project_id, idx, meta)
    return {"ok": True, "scene": st.db.get_scene(project_id, idx),
            "url": f"/api/files?path={os.path.abspath(dst)}",
            "note": "scene still set from the cached image — generation is skipped for this scene"}
