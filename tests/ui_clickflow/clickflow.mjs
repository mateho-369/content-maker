...
// the panel chips toggle (panels stack now, they do not swap), so a step that wants a
// panel VISIBLE has to say so — clicking an already-open chip would close it
const chip = (re, want = true) => {
  const el = $$(".chips .chip").find((b) => re.test(txt(b)));
  if (!el) return null;
  if ((el.getAttribute("data-active") === "1") !== want) click(el);
  return el;
};
const key = (k, init = {}) => window.document.dispatchEvent(new window.KeyboardEvent("keydown", { key: k, bubbles: true, ...init }));
...
(async function main() {
  ...
  chip(/Automated QA Gate/);
  await wait("QA panel", () => panel("Automated QA Gate"));
  ok("30a panels stack — opening the QA gate does not hide the board",
    !!panel("Scene board") && !!panel("Automated QA Gate"),
    $$(".chips .chip").map((c) => `${txt(c).slice(0, 12)}=${c.getAttribute("data-active") === "1"}`).join(" "));
  await tick(600);
  ...
  // (QA + Director were opened above and stay open — the board is a peer panel now,
  //  so this only makes sure it is visible rather than toggling it shut)
  chip(/Storyboard & Production/);
  await tick(250);
  ...
  // Manual mode is "click on everything", so the four load-bearing stages are no
  // longer greyed out. What replaced the lock is a confirm that states the cost, and
  // the run then honestly reports partial — that contract is what is asserted here,
  // in both directions (backing out changes nothing; confirming writes it).
  const core = ["script", "breakdown", "video", "assemble"].map((k) => mcp.querySelector(`[data-testid="stage-run-${k}"]`));
  const coreMarks = core.map((b) => txt(b.closest("label")).match(/needs a confirm|skipped[^·]*/)?.[0] || "");
  ok("37 the four stages a cut needs are clickable in manual mode, each flagged",
    stageBoxes.length >= 8 && core.every((b) => b && !b.disabled && b.checked)
    && coreMarks.every((m) => /confirm/i.test(m)),
    `${stageBoxes.length} stages · marks: ${coreMarks.join(" | ")}`);

  const vidBox = mcp.querySelector('[data-testid="stage-run-video"]');
  click(vidBox); await tick(320);
  const askEl = window.document.querySelector('[data-testid="confirm-core-off"]');
  const askModal = askEl ? askEl.closest(".modal") : null;
  ok("37a switching a core stage off asks what it will cost", !!askModal
    && /MP4 is not produced/.test(txt(askModal)) && /partial/.test(txt(askModal)),
    askModal ? txt(askModal).replace(/\s+/g, " ").slice(0, 96) : "no confirm dialog");
  if (askModal) {
    click(findBtn(askModal, "leave it on")); await tick(300);
    ok("37b backing out writes nothing", vidBox.checked
      && !((await (await realFetch(`${BASE}/api/projects/${pid}`)).json()).project.settings.skip_stages || []).includes("video"),
      `video still on=${vidBox.checked}`);
    click(vidBox); await tick(300);
    click(window.document.querySelector('[data-testid="confirm-core-off"]')); await tick(900);
    const p37 = (await (await realFetch(`${BASE}/api/projects/${pid}`)).json()).project;
    const lock = txt(vidBox.closest("label"));
    ok("37c confirming it is stored and the row says what it means",
      (p37.settings.skip_stages || []).includes("video") && /skipped · no cut/.test(lock),
      `skip_stages=${JSON.stringify(p37.settings.skip_stages)} · ${lock.slice(0, 42)}`);
    click(vidBox); await tick(700);
    const p37b = (await (await realFetch(`${BASE}/api/projects/${pid}`)).json()).project;
    ok("37d ticking it back on clears it again (no confirm needed to restore)",
      !(p37b.settings.skip_stages || []).includes("video"), JSON.stringify(p37b.settings.skip_stages));
  } else {
    for (const t of ["37b backing out writes nothing", "37c confirming it is stored and the row says what it means",
                     "37d ticking it back on clears it again (no confirm needed to restore)"])
      ok(t, false, "no confirm dialog to drive");
  }
  ...
  // ── the single-screen workspace: identity editing, rail filter, assets, ⌘K ──
  {
    const idToggle = findBtn(window.document.querySelector(".main"), "edit");
    click(window.document.querySelector('[data-testid="identity-toggle"]'));
    await tick(420);
    const titleBox = window.document.querySelector('[data-testid="id-title"]');
    ok("49 project details open inline in the workspace (no wizard detour)", !!titleBox
      && !!window.document.querySelector('[data-testid="id-duration"]')
      && !!window.document.querySelector('[data-testid="id-notes"]'),
      titleBox ? `title=${JSON.stringify(titleBox.value).slice(0, 30)}` : "fields missing");
    if (titleBox) {
      setValue(titleBox, titleBox.value + " · retitled");
      await tick(1400);                                    // debounced PATCH + reload
      const p49 = (await (await realFetch(`${BASE}/api/projects/${pid}`)).json()).project;
      ok("49a the retyped title is on the project, not just in the input",
        p49.title.endsWith("· retitled"), p49.title);
      const dur = window.document.querySelector('[data-testid="id-duration"]');
      setValue(dur, 41);
      await tick(1400);
      const p49b = (await (await realFetch(`${BASE}/api/projects/${pid}`)).json()).project;
      ok("49b target length writes through to the render plan", p49b.target_duration === 41,
        `target_duration=${p49b.target_duration}`);
      setValue(dur, 30); await tick(900);                   // leave it where the run expects
      const notes = window.document.querySelector('[data-testid="id-notes"]');
      ok("49c every field the render reads is present", !!notes && !!window.document.querySelector('[data-testid="id-type"]')
        && !!window.document.querySelector('[data-testid="id-voice"]'), "type/notes/voice");
    }

    // assets panel: the two features the deleted pages owned, now writing scene fields
    chip(/Assets & sounds/);
    await tick(700);
    const sfxRows = $$('[data-testid^="sfx-use-"]');
    ok("50 the sfx browser lists real sounds from the shared folder", sfxRows.length >= 3,
      `${sfxRows.length} sounds`);
    if (sfxRows.length) {
      const name = sfxRows[0].getAttribute("data-testid").replace("sfx-use-", "");
      click(window.document.querySelector(`[data-testid="scene-in-run-0"]`)); // make sure a row is selected
      onSel0();
      click(sfxRows[0]);
      await tick(1600);
      const sc = (await (await realFetch(`${BASE}/api/projects/${pid}/scenes`)).json()).scenes;
      ok("50a picking a sound writes sfx_prompt on the scene the render reads",
        sc.some((x) => (x.sfx_prompt || "") === name), `wanted “${name}”, got: `
        + sc.slice(0, 2).map((x) => `${x.idx}:${x.sfx_prompt || "—"}`).join(" ")
        + (toasts().length ? ` · toast: ${toasts().join(" / ").slice(0, 120)}` : ""));
      ok("50b the board's other rows were not wiped by that single-field write",
        sc.length === 15, `${sc.length} scenes after an sfx pick`);
    }
    const imgSearch = window.document.querySelector('[data-testid="img-search"]');
    ok("50c the still search is wired to the server (offline-safe path)", !!imgSearch
      && !imgSearch.disabled, imgSearch ? "present" : "absent");

    function onSel0() { /* the rail keeps selection inside ProjectView; clicking row 1 is enough */
      const first = rows(panel("Scene board"))[0];
      if (first) click(first.querySelector("td"));
    }

    // the rail is a filter, not a heading
    const before = $$('[data-testid^="rail-p"]').length;
    setValue(window.document.querySelector('[data-testid="rail-search"]'), "zzz-no-such-project");
    await tick(700);
    const after = $$('[data-testid^="rail-p"]').length;
    ok("51 the rail's search filters against the API (no client-side theatre)",
      after < before, `${before} rows → ${after} rows`);
    setValue(window.document.querySelector('[data-testid="rail-search"]'), "");
    await tick(700);

    // ⌘K: one way to point at anything
    click(window.document.querySelector('[data-testid="open-palette"]'));
    await tick(240);
    const pin = window.document.querySelector('[data-testid="palette-input"]');
    ok("52 ⌘K opens a command palette", !!pin, pin ? "input ready" : "no palette");
    if (pin) {
      setValue(pin, "Clip");
      await tick(260);
      const items = $$('[data-testid="palette-list"] .palette-item').map((b) => txt(b));
      ok("52a it filters to the thing you typed", items.length > 0 && items.every((t) => /clip/i.test(t)),
        items.join(" | ").slice(0, 90));
      key("Escape"); await tick(200);
      ok("52b Esc closes it", !window.document.querySelector('[data-testid="palette-input"]'), "closed");
    }
  }

  // 12 ─ sweep every nav tab for hard errors ----------------------------------
  // the side rail is one screen now: every item opens a section of the drawer, so the
  // sweep asserts the DRAWER renders each one (an empty or crashing section is the
  // failure this check has always been about)
  const tabs = $$(".side .nav").map((b) => txt(b));
  let tabErr = "", empty = "";
  for (const t of tabs) {
    const b = $$(".side .nav").find((x) => txt(x) === t);
    click(b);
    // some sections fetch their data on open, so wait for content rather than
    // failing on the first paint (a spinner is not an empty panel)
    const dr = await wait("drawer content for " + t, () => {
      const el = window.document.querySelector('[data-testid="drawer"]');
      return el && txt(el).length > 24 ? el : null;
    }, 4000).catch(() => null);
    const body = dr ? txt(dr) : "";
    if (!dr) empty += t + " ";
    if (/Cannot read|Minified React error|is not a function/.test(body)) tabErr += t + " ";
  }
  ok("35 all " + tabs.length + " sections open and render without a crash",
    !tabErr && !empty, [tabErr && "crash: " + tabErr, empty && "empty: " + empty].filter(Boolean).join(" ") || `${tabs.length} sections ok`);
  click(window.document.querySelector('[data-testid="drawer"] .ghost')); await tick(180);

  // ---------------------------------------------------------------- summary
  // Printed as a list, not as a claim: a summary that says "the two expected" while
  // five requests refused is exactly the kind of reassuring text this project is about.
  const fours = net.filter((n) => n.s >= 400 && n.s < 500 && n.m !== "MARK");
  const unexpected = net.filter((n) => n.s >= 500);
  console.log(`\n\x1b[1mnetwork:\x1b[0m ${net.filter((n) => n.m !== "MARK").length} requests · `
    + `${fours.length} returned 4xx · ${unexpected.length} returned 5xx`);
  if (fours.length) console.log("      each refusal: "
    + fours.map((n) => `${n.m} ${n.p} → ${n.s}`).join("  |  "));
  if (unexpected.length) console.log("\x1b[31m      each 5xx: \x1b[0m"
    + unexpected.map((n) => `${n.m} ${n.p} → ${n.s}`).join("  |  "));
