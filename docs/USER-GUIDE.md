...
Open **http://localhost:8000** to start. There is one screen:

* **left rail** — `+ New project`, a filter box, the project list, and below it the
  sections (Gallery, Clip a video, History, Services, AI Team, Characters, Voices,
  Plugins/Engines, Settings, Memory);
* **top bar** — the project you are in (a picker, so you can swap without losing your
  place), the machine/plan status pill, the TTS voice pill, and ⌘K;
* **the workspace** — the open project's panels. They *stack*, they don't swap: the
  chips above them (`Storyboard & manual control`, `Director & script`, `Pipeline & QA`,
  `Assets & sounds`) decide which are mounted, and `⌘1…⌘4` toggle them.

Sections open in a slide-over drawer, so a project you were editing stays exactly where
it was. `#/<section>` and `#/project/<id>` are real addresses, so a link still points at
something.
